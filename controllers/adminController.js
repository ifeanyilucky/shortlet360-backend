...
const getDashboardStats = async (req, res) => {
  try {
    ...
    const recentBookings = await Booking.find()
      .sort({ createdAt: -1 })
      .limit(5)
      .select("check_in_date check_out_date total_price booking_status payment_status createdAt")
...
const updateUser = async (req, res) => {
  const { id } = req.params;
  const { first_name, last_name, email, phone_number, business_name } = req.body;
...
const getPropertyById = async (req, res) => {
  ...
  const property = await Property.findById(id).populate("owner", "first_name last_name email phone_number short_id");
